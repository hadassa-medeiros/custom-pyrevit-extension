...
class RevitDocInterface:
    ...
    def rooms(self):
        return DB.FilteredElementCollector(self.doc).OfCategory(
            self.category_map["rooms"]
        ).ToElements()
    
    @property
    def ceiling_types(self):
        return DB.FilteredElementCollector(self.doc).OfClass(
            self.category_map["ceiling_types"]
        ).ToElements()
...
def get_name(element):
    try:
        if str(element.Category.BuiltInCategory) == 'OST_Rooms':
            name_param = element.get_Parameter(DB.BuiltInParameter.ROOM_NAME)
            return name_param.AsString() if name_param and name_param.HasValue else "Ambiente sem nome"
        # else:
        name_param = element.get_Parameter(DB.BuiltInParameter.ALL_MODEL_TYPE_NAME)
        if name_param and name_param.HasValue:
            return name_param.AsString()

        # Caso ALL_MODEL_TYPE_NAME não exista ou não tenha valor, tenta ELEM_TYPE_PARAM
        type_param = element.get_Parameter(DB.BuiltInParameter.ELEM_TYPE_PARAM)
        if type_param and type_param.HasValue:
            return type_param.AsValueString()  # Ou AsString(), dependendo do formato

        # Retorno padrão se nenhum parâmetro for encontrado
        return "Elemento sem nome"

    except Exception as e:
        print("Erro ao obter o nome do elemento ID {}: {}".format(element.Id.AsInteger, e))
        return "Erro ao acessar nome do elemento"
...
def get_category(element):
    return str(element.Category.BuiltInCategory)

class GetCeilingHeight:
    def __init__(self, roomElement):
        self.roomElement = roomElement
        if get_category(roomElement) == 'OST_Rooms':
            self.parameter = roomElement.LookupParameter('PE DIREITO').AsDouble()
        else:
            self.parameter = None
    @property
    def metric(self):
        if self.parameter and self.parameter.HasValue():
            return self.parameter.AsValueString()
        # else: 
        #     return 'PD não informado para {}'.format(self.roomElement)

def metric_to_double(value):
    metric_to_double_factor = 3.280840
    value_in_double = round(value * metric_to_double_factor, 5)
