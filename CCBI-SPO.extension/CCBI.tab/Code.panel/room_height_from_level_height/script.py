...
for selected_room_number in selected_room_numbers:
    ...
    for level in levels:
        try:
            ...
            new_rooms_upper_offset_metric = new_rooms_upper_offset/double_to_meter_divisor
            if room.Level.Id == level.Id and floor_to_floor_height > 0:
                ...
                t.Commit()
                print("Upper offset parameter value of the room {} has been modified to {}m.".format(room_name, round(new_rooms_upper_offset_metric,3)))
        except IndexError:
            pass