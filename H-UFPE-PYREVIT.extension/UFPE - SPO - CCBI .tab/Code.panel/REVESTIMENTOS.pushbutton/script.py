...
for room in rooms:
    # Shared parameters created to store room's finish materials
    cod_walls = room.LookupParameter('COD-REV-PAREDE_1')
    cod_walls2 = room.LookupParameter('COD-REV-PAREDE_2')
    # cod_walls3 = room.LookupParameter('COD-REV-PAREDE_3')
    cod_floor = room.LookupParameter('COD-REV-PISO_1')
    cod_floor2 = room.LookupParameter('COD-REV-PISO_2')
    cod_ceiling = room.LookupParameter('COD-REV-TETO_1')
    ...
    rooms_ceiling_finish = room.get_Parameter(DB.BuiltInParameter.ROOM_FINISH_CEILING)
    rooms_wall_finish = room.get_Parameter(DB.BuiltInParameter.ROOM_FINISH_WALL)
    rooms_floor_finish = room.get_Parameter(DB.BuiltInParameter.ROOM_FINISH_FLOOR)
    room_bbox = room.get_BoundingBox(doc.ActiveView)
    room_outline = DB.Outline(room_bbox.Min, room_bbox.Max)  # é a outline que se passa como arg pro método DB.BoundingBoxIntersectsFilter (e não um objeto tipo BoundingBoxXYZ
                ...
                for layer in layers:
                    ...
                    if elem_category == walls_category and is_layer_finish:
                        ...
                        if any(possibility == True for possibility in ceiling_possibilities):
                            ...
                            t.Start()
                            cod_ceiling.Set(ceiling_material.Id)
                            rooms_ceiling_finish.Set(material_mark)
                            t.Commit()
                        ...
                        if is_layer_finish:
                            ...
                            t.Start()
                            cod_floor.Set(floor_material.Id)
                            rooms_floor_finish.Set(material_mark)
                            t.Commit()
    ...
    if len(wall_materials_ids_list) == 1:
        ...
        t.Start()
        cod_walls.Set(wall_materials_ids_list[0])
        rooms_wall_finish.Set(material_mark)
        t.Commit()
        ...
        t3.Start()
        cod_walls.Set(wall_materials_ids_list[0])
        cod_walls2.Set(wall_materials_ids_list[1])
        rooms_wall_finish.Set(material_mark)
        # cod_walls3.Set(wall_materials_list[2])
        t3.Commit()
