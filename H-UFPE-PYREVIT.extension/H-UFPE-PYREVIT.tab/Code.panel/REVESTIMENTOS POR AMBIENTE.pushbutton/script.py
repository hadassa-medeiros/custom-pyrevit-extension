...
# niveis_collector = revit.FilteredElementCollector(doc).OfCategory(revit.BuiltInCategory.OST_Levels).ToElements()
...
#     desloc_limite = ambiente.get_Parameter(revit.BuiltInParameter.ROOM_UPPER_OFFSET)
#     lim_sup = ambiente.get_Parameter(revit.BuiltInParameter.ROOM_UPPER_LEVEL)
#
#     mudar_offset_room = revit.Transaction(doc, "aplicar offset")
#     mudar_offset_room.Start()
#
#     print(lim_sup.AsElementId())
#     # novo_valor_desloc_limite = revit.UnitUtils.ConvertFromInternalUnits(desloc_limite, revit.UnitUtils.DisplayUnitType.DUT_METERS)
                    ...
                    if funcao == 'Finish2' or funcao == 'Membrane':
                        try:
                            if element.Category.Name == 'Paredes':
                                ...
                                material_rev_parede = doc.GetElement(revit.ElementId(int(camada.MaterialId.ToString())))
                                ...
                                material_rev_piso = doc.GetElement(revit.ElementId(int(camada.MaterialId.ToString())))
                                ...
                                material_rev_forro = doc.GetElement(revit.ElementId(int(camada.MaterialId.ToString())))
                                print(material_rev_forro.Name, element.Name)
                                for material in materiais:
                                    if material.Id == camada.MaterialId:
                                        revest_forro = material
                                        print('ok, material associado a FORRO:', revest_forro.Name)
                                t = revit.Transaction(doc, "aplicar cod revest forro a ambiente")
                                t.Start()
